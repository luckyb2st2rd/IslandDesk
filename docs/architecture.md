...
The first vertical slice is the island shell, including state transitions, system tray, settings shell, persistent settings and multi-monitor positioning. The second slice adds Windows media metadata and transport commands. The expanded module navigation now exposes all seven product sections. File Shelf provides native drag-in and cross-application drag-out, deduplication, reference-only persistence, pinning, missing-file detection, relinking and metadata-only removal. Clipboard History provides event-driven Windows capture, encrypted persistence, retention, pinning, deletion, pause and source-application exclusions. Notes are session-only and the timer is local.
