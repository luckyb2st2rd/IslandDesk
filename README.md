...
The first implementation target is the island shell: a borderless, transparent, always-on-top Windows window with `Collapsed` and `Expanded` states, tray control, a settings shell, SQLite configuration, DPI-aware multi-monitor placement, edge-triggered auto-hide, and automatic fullscreen suppression. The typed Flutter-Rust bridge is active through Flutter Native Assets. Windows adapters stream native Global System Media Transport Controls changes, expose Core Audio endpoint volume/mute controls, hold Keep Awake requests on a dedicated native thread, and launch user-selected applications through the Windows shell.
